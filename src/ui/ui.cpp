...
bool UI::validInput1Cmd2ArgsDigit(const std::string &input, const std::string &cmd, bool acceptFloat) {
    std::vector<std::string> words = to_words(input);
    return words.size() == 3 && words.at(0) == cmd
           && isdigit(words.at(1),acceptFloat) && isdigit(words.at(2),acceptFloat);
}
